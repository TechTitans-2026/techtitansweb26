...
export default function Home() {
  ...
  const messageWords = feedbackData.message.trim() ? feedbackData.message.trim().split(/\s+/).filter(Boolean) : [];
  ...
  const handleSubmitFeedback = async (e) => {
    ...
    if (isOverLimit) {
      setFeedbackStatus(`Error: Message exceeds 60-word limit (${wordCount}/60 words).`);
      return;
    }
    ...
    try {
      const { error } = await supabase.from('feedback').insert([{
        name: feedbackData.name.trim() || profile?.full_name || user?.user_metadata?.full_name || 'Titan Operative',
  ...
  return (
    <div className={`home-body transition-opacity duration-1000 ease-out ${isEntering ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div id="view-home" className="page-view active">
          <section className="relative pt-6 pb-16 lg:pt-16 lg:pb-32 overflow-hidden">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 text-center w-full">
              ...
              <div className="code-window w-full max-w-2xl mx-auto rounded-xl overflow-hidden mb-8 sm:mb-16">
                ...
                <div className="p-5 sm:p-10 md:p-12 flex items-center justify-center relative overflow-hidden">
                  ...
                  <div className="font-mono text-xs sm:text-xl md:text-2xl font-semibold tracking-tight relative z-10">
                    ...
                    <span className="text-[#f1fa8c]" id="typewriter">{phrases[phraseIdx]}</span>
                    <span className="text-gray-300">)</span>
              ...
              {/* Dynamic Action Buttons: Sign In when guest, Centered Explore Events when logged in */}
              {!user ? (
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5 px-2 sm:px-0">
                  <Link to="/auth" className="btn-keycap px-10 py-3.5 text-sm w-full sm:w-auto text-center inline-flex items-center justify-center min-h-[48px] box-border m-0">
                    Sign In
                  </Link>
                  <button onClick={() => navigate('/events')} className="bg-transparent border border-white/20 hover:border-white/50 hover:bg-white/5 text-white font-bold rounded-lg px-10 py-3.5 text-sm w-full sm:w-auto transition-all min-h-[48px] flex items-center justify-center box-border m-0 cursor-pointer">
                    Explore Events
                  </button>
                </div>
              ) : (
                <div className="flex items-center justify-center px-2 sm:px-0">
                  <button onClick={() => navigate('/events')} className="btn-keycap px-12 py-3.5 text-sm w-full sm:w-auto text-center inline-flex items-center justify-center min-h-[48px] box-border m-0 cursor-pointer">
                    Explore Events
                  </button>
                </div>
              )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 relative z-10 flex flex-col gap-16 sm:gap-24">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16">
              ...
              <div className="reveal">
                <h2 className="text-2xl section-title mb-6 sm:mb-8">Our Story &amp; Purpose</h2>
                <div className="glass-panel p-6 sm:p-8 relative h-full flex flex-col justify-center overflow-hidden">
                  <i className="fas fa-check-circle absolute -top-3 -right-3 text-3xl text-accent bg-[#1a1b22] rounded-full shadow-lg"></i>
                  ...
                  <p className="text-[#8c8d96] text-sm sm:text-base leading-relaxed mb-6">
                    Tech Titans was formed to bridge the gap between academic theory and high-impact engineering. We are a collective of developers, designers, and innovators focused on building real-world software and exploring decentralized technologies.
                  </p>
              ...
              {/* Latest Updates Feed */}
              <div className="reveal">
                <h2 className="text-2xl section-title mb-6 sm:mb-8">Latest Updates</h2>
                <div className="space-y-4">
                  {latestEvent ? (
                    <div className="glass-panel p-5 sm:p-6 flex items-start gap-4 hover:border-accent/40 transition-colors">
                      ...
                      <div>
                        <span className="text-xs text-accent font-mono uppercase tracking-wider block mb-1">Upcoming Event</span>
                        <h3 className="font-bold text-white text-base sm:text-lg mb-1">{latestEvent.title}</h3>
                        <p className="text-[#8c8d96] text-xs sm:text-sm line-clamp-2">{latestEvent.description}</p>
                      </div>
                    </div>
                  ) : (
                  ...
                  {latestQuest && (
                    <div className="glass-panel p-5 sm:p-6 flex items-start gap-4 border border-[#00f3ff]/20 hover:border-[#00f3ff]/50 transition-colors">
                      ...
                      <div>
                        <span className="text-xs text-[#00f3ff] font-mono uppercase tracking-wider block mb-1">Active Bounty</span>
                        <h3 className="font-bold text-white text-base sm:text-lg mb-1">{latestQuest.title}</h3>
                        <p className="text-[#8c8d96] text-xs sm:text-sm line-clamp-2">{latestQuest.description}</p>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* Footer */}
      <footer id="contact-footer" className="border-t border-white/5 bg-[#16171d] py-12 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col gap-10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="text-center md:text-left">
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight mb-2">CONNECT WITH THE NETWORK</h2>
              <p className="text-[#8c8d96] text-xs sm:text-sm max-w-md">
                Have questions about joining, partnering, or sponsoring upcoming events? Drop a line to the core team.
              </p>
            </div>

            <div className="flex gap-6 text-2xl text-[#8c8d96]">
              <a href="mailto:contact@techtitans.club" className="hover:text-accent hover:-translate-y-2 transition-all drop-shadow-md" aria-label="Email"><i className="fas fa-envelope"></i></a>
              <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="hover:text-accent hover:-translate-y-2 transition-all drop-shadow-md" aria-label="LinkedIn"><i className="fab fa-linkedin"></i></a>
              <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="hover:text-accent hover:-translate-y-2 transition-all drop-shadow-md" aria-label="GitHub"><i className="fab fa-github"></i></a>
              <a href="https://discord.com" target="_blank" rel="noopener noreferrer" className="hover:text-accent hover:-translate-y-2 transition-all drop-shadow-md" aria-label="Discord"><i className="fab fa-discord"></i></a>
            </div>

            <button id="msg-btn" className="btn-keycap px-8 py-3.5 text-sm w-full md:w-auto min-w-[200px]" onClick={handleOpenFeedback}>
              SEND MESSAGE
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-[#8c8d96]/60 text-xs font-medium text-center md:text-left">
            <div className="flex flex-col items-center md:items-start gap-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="font-black tracking-tighter text-white/70 text-base sm:text-lg">TECH TITANS</span>
              </div>
      ...
      {showFeedbackForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowFeedbackForm(false)}>
          ...
          >
            ...
            <div className="mb-5">
              <span className="text-accent font-mono text-xs font-bold uppercase tracking-[0.3em] mb-1 block">DIRECT TRANSMISSION</span>
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Send Message</h2>
            </div>
            
            {feedbackStatus && (
              <div className={`p-3 rounded-lg mb-4 text-xs font-mono text-center ${feedbackStatus.includes('Error') ? 'bg-red-500/10 border border-red-500/50 text-red-400' : 'bg-[#ae97d6]/10 border border-[#ae97d6]/50 text-[#ae97d6]'}`}>
                {feedbackStatus}
              </div>
            )}

            <form onSubmit={handleSubmitFeedback} className="flex flex-col gap-4 relative z-10">
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-wider font-mono mb-1.5 block">Operative Name</label>
                <input 
                  type="text" 
                  required 
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-mono">Transmission</label>
                  <span className={`text-[11px] font-mono ${isOverLimit ? 'text-red-400 font-bold' : 'text-[#8c8d96]'}`}>
                    {wordCount} / 60 words
                  </span>
                </div>
                <textarea 
                ...
                {isOverLimit && (
                  <p className="text-xs text-red-400 font-mono mt-1">
                    Exceeded by {wordCount - 60} word{wordCount - 60 > 1 ? 's' : ''}. Please trim down to 60 words.
                  </p>
                )}
              </div>
